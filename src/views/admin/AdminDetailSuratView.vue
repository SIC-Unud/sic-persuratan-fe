<template>
  <div>
    <div class="flex justify-between items-center">
      ...
      <div class="flex justify-between">
        <div class="hidden md:flex justify-end gap-2 bg-white">
          <RouterLink
            to="/admin/surat"
            class="px-4 py-2 bg-white text-primary text-xl font-bold rounded-xl border border-primary hover:shadow-xl transition"
          >
            Kembali
          </RouterLink>
          <RouterLink
            v-if="surat"
            :to="`/admin/surat/${surat.id}/update-surat`"
            class="px-6 py-2 bg-primary text-white text-xl font-bold rounded-xl hover:shadow-xl transition"
          >
            Update
          </RouterLink>
        </div>
      ...
      <div class="flex gap-6">
        <div class="flex flex-col w-full">
          ...
          <div class="p-3 bg-white rounded-md">
            <p class="font-medium text-black">{{ surat.tanggal }}</p>
          </div>
        </div>
        <div class="flex flex-col w-full">
    ...
    <div class="flex md:hidden justify-end gap-3 bg-white py-6">
      <RouterLink
        to="/admin/surat"
        class="text-center px-4 py-2 bg-white text-primary text-xl font-bold rounded-xl border border-primary hover:shadow-xl transition"
      >
        Kembali
      </RouterLink>
      <RouterLink
        v-if="surat"
        :to="`/admin/surat/${route.params.id}/update-surat`"
        class="text-center px-4 py-2 bg-primary text-white text-xl font-bold rounded-xl hover:shadow-xl transition"
      >
        Update
      </RouterLink>
    </div>
