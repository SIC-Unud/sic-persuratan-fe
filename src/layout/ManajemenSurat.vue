<template>
  <main class="h-full w-full min-h-[83vh] font-jakarta-sans md:col-span-9">
    ...
    <div class="overflow-x-auto">
      ...
      <!-- Table Body -->
      <div
        v-for="(surat, index) in suratList"
        :key="index"
      ...
      >
        ...
        <p class="hidden w-2/12 md:block">{{ surat.namaKegiatan }}</p>
        ...
        <div class="hidden w-1/12 items-center justify-center gap-1 py-[10px] md:flex md:gap-1">
          <RouterLink to="{ name: 'DetailSurat', params: { id: surat.nomorSurat } }">
            <button>
...
const suratList = ref([
  {
    nomorSurat: 'XX/XXXX/XXXX',
    sumberSurat: 'Surat Panitia Pelaksana',
    jenisSurat: 'Surat Permohonan Ijin Kegiatan',
    namaKegiatan: 'Militerisasi',
    status: 'Menunggu diajukan',
  },
  {
    nomorSurat: 'X/XXXX/XXXX',
    sumberSurat: 'Surat Panitia Pelaksana',
    jenisSurat: 'Surat Permohonan Ijin Kegiatan',
    namaKegiatan: 'Melukat',
...
const totalSurat = suratList.value.length
</script>
